...
def get_train_org(*args):
    """Get original training dataset for KDD 10 percent"""
    return _get_adapted_dataset("train_org")
...
def load_data(SET_NUM):
    dir = '/home/xlyang/kaggle/icdm/aircraft/train_test_data_features/aircraft_{}'.format(SET_NUM)
    X_train = np.load(dir + '/train_data.npy')
    Y_train = np.load(dir + '/train_label.npy')
    X_test = np.load(dir + '/test_data.npy')
    Y_test = np.load(dir + '/test_label.npy')
...
def _get_dataset():
    ...
    X_train, y_train, X_test, y_test = load_data(4)
    ...
    dataset['x_train_org'] = x_train_org.astype(np.float32)
    dataset['y_train_org'] = y_train_org.astype(np.float32)
